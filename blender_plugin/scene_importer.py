"""
SceneImporter — shared import codepath for both native .skp files and live
JSON snapshots (via the duck-typed JsonModel adapter).
...
# Cython SLAPI bindings — optional, only needed for native .skp file import
try:
...
class SceneImporter:
    ...
    def load(self, context, **options):
        ...
        # LiveImportSKP sets self.skp_model before calling load(), so this
        # block is skipped for live imports and the file-path branch is
        # unchanged for regular file imports.
        if not hasattr(self, "skp_model") or self.skp_model is None:
            if not _has_sketchup:
                raise RuntimeError(
                    "SketchUp SDK extension is not available. "
                    "Use live import, or build the Cython extension."
                )
            try:
...
class LiveImportSKP(Operator):
    """Import the currently open SketchUp model via sketchup-link (live)"""

    bl_idname = "import_scene.skp_live"
    ...
    reuse_material: BoolProperty(name="Use Existing Materials", default=True)
    dedub_only: BoolProperty(name="Groups Only", default=False)
    reuse_existing_groups: BoolProperty(name="Reuse Groups", default=False)
    max_instance: IntProperty(name="Instantiation Threshold", default=1)
    dedub_type: EnumProperty(
        name="Instancing Type",
        items=(("FACE", "Faces", ""), ("VERTEX", "Vertices", "")),
        default="VERTEX",
    )
    ...
    def execute(self, context):
        ...
        options = dict(
            ...
            dedub_only=self.dedub_only,
            reuse_existing_groups=self.reuse_existing_groups,
            max_instance=self.max_instance,
            dedub_type=self.dedub_type,
            import_scene=self.import_scene,
        )
        return importer.load(context, **options)
    ...
    def draw(self, context):
        ...
        layout.prop(self, "dedub_type")
        layout.prop(self, "import_scene")
...
def menu_func_live_import(self, context):
    self.layout.operator(LiveImportSKP.bl_idname, text="SketchUp Live (sketchup-link)")
